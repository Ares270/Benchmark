"""Audit every raw WarmMolGenOne draw without selecting docking candidates."""
...
GEN2_ANALYSIS_SCHEMA_VERSION = 1


class Gen2AnalysisError(ValueError):
    """Raised when Gen2 sample, intake, or provenance records disagree."""
...
def _read_json(path: Path) -> dict:
    try:
        ...
        raise Gen2AnalysisError(f"Cannot read {path}: {error}") from error
    if not isinstance(value, dict):
        raise Gen2AnalysisError(f"{path} must contain one JSON object")
    return value


def _verify_record(path: Path, record: dict, label: str) -> None:
    if not path.is_file():
        raise Gen2AnalysisError(f"{label} does not exist: {path}")
    ...
    if not recorded_hash or runtime.sha256_file(path) != recorded_hash:
        raise Gen2AnalysisError(f"{label} does not match its recorded SHA-256: {path}")
...
def _parent_key(parent_smiles: str) -> str:
    ...
    if molecule is None:
        raise Gen2AnalysisError(f"Accepted Gen2 parent no longer parses: {parent_smiles}")
    try:
        with rdBase.BlockLogs():
            value = str(inchi.MolToInchiKey(molecule) or "")
    except Exception as error:
        raise Gen2AnalysisError(
            f"Could not compute an InChIKey for accepted Gen2 parent {parent_smiles}"
        ) from error
    if not value:
        raise Gen2AnalysisError(
            f"RDKit returned an empty InChIKey for accepted Gen2 parent {parent_smiles}"
        )
    return value
...
def _sampling_inputs(sample_dir: Path) -> tuple[dict, pd.DataFrame]:
    ...
    if sampling.get("stage") != "gen2_sampling" or sampling.get("model_name") != MODEL_NAME:
        raise Gen2AnalysisError(f"{sampling_path} is not a Gen2 sampling record")
    outputs = sampling.get("outputs", {})
    _verify_record(raw_path, outputs.get("raw_samples_csv", {}), "Raw Gen2 samples")
    _verify_record(
        molecules_path,
        outputs.get("molecules_smi", {}),
        "Gen2 transport SMILES",
    )
    raw = pd.read_csv(
        raw_path,
    ...
    if missing:
        raise Gen2AnalysisError(f"Raw Gen2 samples are missing columns: {sorted(missing)}")
    if len(raw) != sampling.get("counts", {}).get("raw_samples"):
        raise Gen2AnalysisError("Raw Gen2 sample count does not match sampling.json")
    ...
    if not np.array_equal(observed_indices, expected_indices):
        raise Gen2AnalysisError("Raw Gen2 sample_index is not the exact 1..N sequence")
    if raw["molecule_id"].duplicated().any():
        raise Gen2AnalysisError("Raw Gen2 molecule IDs are not unique")
    return sampling, raw
...
def _intake_inputs(
    ...
    if summary.get("stage") != "molecule_intake":
        raise Gen2AnalysisError(f"{summary_path} is not a molecule-intake record")
    if summary.get("input", {}).get("sha256") != runtime.sha256_file(
        sample_dir / MOLECULES_NAME
    ):
        raise Gen2AnalysisError("Full-cohort intake is not linked to the Gen2 sample file")
    if summary.get("counts", {}).get("submitted_rows") != expected_rows:
        raise Gen2AnalysisError("Full-cohort intake did not receive every raw Gen2 draw")
    ...
    if not molecules_csv.is_file() or runtime.sha256_file(molecules_csv) != recorded_hash:
        raise Gen2AnalysisError("Full-cohort molecules.csv fails its intake hash")
    frame = pd.read_csv(
        molecules_csv,
...
def build_gen2_profile(
    sample_dir: Path,
    intake_dir: Path,
    actives_path: Path,
) -> tuple[pd.DataFrame, dict]:
    """Build the full-cohort profile without selecting or ranking molecules."""
    ...
    if accepted.empty:
        raise Gen2AnalysisError("Gen2 full-cohort intake accepted no parent structures")
    ...
    accepted["exact_known_active_parent"] = accepted["parent_inchikey"].isin(
        active_keys
    )
    ...
    profile = {
        "schema_version": GEN2_ANALYSIS_SCHEMA_VERSION,
        "stage": "gen2_full_cohort_analysis",
        "model_name": MODEL_NAME,
        "scope": "all raw Gen2 draws; no docking subsample selection is used here",
        "interpretation": {
            "target_aware": True,
            "conditioning": "locked DYRK1A Q13627 kinase-domain sequence only",
        ...
        "generation_quality": {
            ...
            "training_novelty": {
                "available": False,
                "reason": (
                    "No authenticated WarmMolGenOne BindingDB training-parent and "
                    "scaffold index is available; training-set novelty is not guessed."
                ),
            },
        },
        "chemistry": {
...
def _render_report(profile: dict) -> str:
    ...
    novelty_text = quality["training_novelty"]["reason"]
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Gen2 full-cohort analysis</title>
<style>{report_css()}</style></head>
<body>{report_toolbar("Gen2 full-cohort audit")}<h1>Gen2: target-aware WarmMolGenOne</h1>
<p>This section covers all {sampling['raw_samples']:,} raw decoder draws before screening selection. The frozen model received only the {target['input_sequence_length']}-residue DYRK1A Q13627 kinase-domain sequence. It received no active ligands, receptor coordinates, docking grid, gate result, or Smina score.</p>
...
<div class="card"><h2>Decoder behavior</h2><p>EOS terminated: {sampling['terminated_by_eos']:,}; hit the 128-token ceiling: {sampling['hit_max_length']:,}; empty raw SMILES: {sampling['empty_raw_smiles']:,}. No invalid or duplicate draw was replaced.</p></div>
<div class="card"><h2>Training-novelty boundary</h2><p>{html.escape(novelty_text)}</p></div>
...
def write_gen2_analysis(
    sample_dir: Path,
    intake_dir: Path,
    actives_path: Path,
    outdir: Path,
) -> Path:
    accepted, profile = build_gen2_profile(sample_dir, intake_dir, actives_path)
    outdir = Path(outdir)
    if outdir.exists():
        raise Gen2AnalysisError(f"Gen2 analysis output already exists: {outdir}")
    ...
    runtime.write_json_atomic(
        ...
        {
            "schema_version": GEN2_ANALYSIS_SCHEMA_VERSION,
            "stage": "gen2_full_cohort_analysis_run_log",
            "outputs": {
